...
resource "aws_security_group" "sg_example_04" {
  name        = sg_example_04
  description = "Security group for instance example 04."
...
resource "aws_instance" "instance_example_04" {
  ...
  user_date = file("scripts/install_docker.sh")

  tags = {
    Name = "instance_example_04"
  }
}
...
output "public_ip" {
  value = aws.instance.instance_example_04.public_ip
}