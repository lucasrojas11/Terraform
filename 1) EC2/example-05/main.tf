...
resource "aws_security_group" "sg_example_05" {
  name        = var.sg_name
...
resource "aws_security_group_rule" "ingress" {
  security-group_id = aws_security_group.sg_example_05.id
  type              = ingress
...
resource "aws_instance" "instance_example_05" {
  ami             = var.ami_id
  instance_type   = var.instance_type
  key_name        = var.key_name
  security_groups = [aws_security_group.sg_example_05.name]
...
resource "aws_eip" "elastic_ip" {
  instance = aws_instance.instance_example_05.id
}
